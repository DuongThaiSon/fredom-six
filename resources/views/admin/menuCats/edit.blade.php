    ...
    <div class="container-fluid" style="background: #e5e5e5;">
        <div id="content">
            <form action="{{ route('admin.menu-categories.update', $category->id) }}" method="POST"
                enctype="multipart/form-data" class="bg-white mb-0 p-4 pt-5">
                @csrf
                @method('PUT')
                {{-- @if ($errors->any())
        ...
        <div class="row">
            <div class="col">
                <div class="form-group">
                    ...
                    <small class="form-text">Tên cho danh mục menu</small>
                </div>
                <div class="form-group">
                    <label class="control-label">Mã menu</label>
                    <span type="text" id="slug" name="slug" value="{{ $category->slug }}" class="form-control"
                        placeholder="Tên danh mục">{{ $category->slug }}</span>
                    <small class="form-text">Mã menu</small>
                </div>
            </div>
        </div>
        </form>
